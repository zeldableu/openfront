...
const { analyze } = require('../team-history.js');
const detail = analyze(result.days.flatMap(day => day.matches));
assert.equal(detail.games, 4);
assert.equal(detail.gains, 2.2);
assert.equal(detail.deductions, -1.5);
assert.equal(detail.wins, 2);
assert.equal(detail.losses, 2);
assert.equal(detail.timed, 0);
assert.equal(detail.best.id, 'a');
assert.equal(detail.worst.id, 'unmatched');
assert.equal(detail.hours.reduce((a,b) => a+b, 0), 4);
assert.equal(summarize([{ gameId: 'null', gameStart: sessions[0].gameStart, score: null }], [], '2026-09-14', '2026-09-14').games, 0);
const streak = analyze([{ start: sessions[0].gameStart, points: 1, won: true, duration: 60, map: 'A', mode: 'Team' }, { start: sessions[1].gameStart, points: 2, won: true, duration: 120, map: 'A', mode: 'Team' }]);
assert.equal(streak.winStreak, 2);
assert.equal(streak.seconds, 180);
assert.equal(streak.maps[0].points, 3);
